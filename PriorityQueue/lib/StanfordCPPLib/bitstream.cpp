...
void obitstream::writeBit(int bit) {
    if (bit != 0 && bit != 1) {
        error(std::string("writeBit must be passed an integer argument of 0 or 1.  You passed the integer ")
...
ifbitstream::ifbitstream(std::string filename) {
    init(&fb);
    open(filename);
}
...
void ifbitstream::open(std::string filename) {
    open(filename.c_str());
}
...
ofbitstream::ofbitstream(std::string filename) {
    init(&fb);
    open(filename);
}
...
void ofbitstream::open(const char* filename) {
    ...
    */
    if (endsWith(filename, ".cpp") || endsWith(filename, ".h") ||
        ...
        error(std::string("It is potentially dangerous to write to file ")
...
void ofbitstream::open(std::string filename) {
    open(filename.c_str());
}
 ...
 */
istringbitstream::istringbitstream(std::string s) {
    init(&sb);
    sb.str(s);
}
 ...
 * specified string.
 */
void istringbitstream::str(std::string s) {
    sb.str(s);
}
