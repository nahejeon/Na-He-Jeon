...
static void echoConsole(const std::string& str);
static void fileLogConsole(const std::string& str);
...
static std::streambuf* cin_old_buf;
static std::streambuf* cout_old_buf;
...
static std::string getResult(bool consumeAcks = false);
...
#endif
            if (result != 0) {
                std::cerr << "Warning: Unable to increase system stack size to "
                     << stackSize << std::endl;
            }
        }
    }
...
static WINBOOL WinCheck(WINBOOL result) {
    if (result == 0 && result != ERROR_IO_PENDING) {
        ...
        FormatMessageA(/* dwFlags */ FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                       ...
                       /* arguments */ NULL);
        if (errorMsg) {
            std::cerr << "Error from Java back-end subprocess: " << errorMsg << std::endl;
        }
    }
    return result;
}
...
void startupMainDontRunMain(int /*argc*/, char** argv) {
    ...
    initPipe();
    cin_old_buf = std::cin.rdbuf();
    cout_old_buf = std::cout.rdbuf();
...
static void initPipe() {
    ...
    if (!ok) {
        ...
        std::cerr << std::endl;
        std::cerr << "ERROR: Stanford C++ library was unable to connect" << std::endl;
        std::cerr << "       to its Java back-end to launch 'spl.jar'." << std::endl;
        std::cerr << "       Please check your Java installation and make sure" << std::endl;
        std::cerr << "       that spl.jar is properly attached to your project." << std::endl;
        std::cerr << std::endl;
...
static bool LinCheck(ssize_t result) {
    if (result == EPIPE) {
        // std::cerr << "Error from Java back-end subprocess." << std::endl;
...
static void sigPipeHandler(int /*signum*/) {
    ...
    std::cerr << "*** Prematurely exiting program because console window was closed." << std::endl;
    std::cerr << "***" << std::endl;
    exit(1);
}
...
static std::string getResult(bool consumeAcks) {
    while (true) {
#ifdef PIPE_DEBUG
...
static std::string getLineConsole() {
    ...
    std::string result = getResult(true);
...
static void fileLogConsole(const std::string& str) {
    std::string consoleLogFile = autograder::getConsoleLogFile();
    if (consoleLogFile.length() > 0) {
        ...
        outfile.open(consoleLogFile.c_str(), std::fstream::out | std::fstream::app);
        outfile << str;
        outfile.flush();
        outfile.close();
    }
